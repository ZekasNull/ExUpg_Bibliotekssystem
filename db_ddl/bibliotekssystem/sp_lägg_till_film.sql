...
BEGIN
    IF array_length(p_regissör_förnamn_arr, 1) IS DISTINCT FROM array_length(p_regissör_efternamn_arr, 1) OR
        ...
        debug_status_code := 1;
        debug_status_message := FORMAT('Fel: Regissörer har [%I] förnamn och [%I] efternamn, Skådespelare har [%I] förnamn och [%I] efternamn.', array_length(p_regissör_förnamn_arr, 1), array_length(p_regissör_efternamn_arr, 1), array_length(p_skådespelare_förnamn_arr, 1), array_length(p_skådespelare_efternamn_arr, 1));
        return;
    END IF;
    ...
    FOR i IN 1..array_length(p_regissör_förnamn_arr, 1) LOOP
            -- Hitta direktören om hen redan finns
            SELECT regissör_id INTO f_regissörid FROM "Regissör"
    ...
    FOR i IN 1..array_length(p_skådespelare_förnamn_arr, 1) LOOP
            SELECT skådespelare_id INTO f_skådespelarid FROM "Skådespelare"
    ...
    -- Lägg till genre
    FOR i IN 1..array_length(p_genrenamn_arr, 1) LOOP
            SELECT genre_id INTO f_genreid FROM "Genre"
